...
export function buildOpenApiDocument() {
  ...
  add("delete", `${A}/users/{userId}/sessions`, { summary: "End all of a user's sessions", description: "Revokes every active session of the user at once, so their access and refresh tokens stop working immediately. Use it after a `device.new` or `risk.challenged` event, or when a user reports a stolen phone.", tag: "Users", auth: "key", scope: "users:write", ok: { status: "200", response: json({ type: "object", properties: { revoked: { type: "integer" } } }) }, extraErrors: { "404": "User not found" } });
  ...
  add("post", `${A}/webhooks/{webhookId}/rotate-secret`, { summary: "Rotate a webhook's signing secret", description: "The new secret is returned once and takes effect immediately: deliveries signed with the old one stop verifying, so update your receiver first or accept both briefly.", tag: "Webhooks", auth: "adminOrKey", scope: "webhooks:manage", ok: { status: "200", response: json({ type: "object", properties: { id: { type: "string" }, secret: { type: "string" } } }) }, extraErrors: { "404": "Not found" } });
  ...
  add("get", `${A}/audit-logs`, { summary: "Audit trail for an application", description: "Who did what and when: key and webhook changes, risk-rule changes. Never contains secrets, full phone numbers or full URLs.", tag: "Audit", auth: "adminOrKey", scope: "audit:read", ok: { status: "200", response: json({ type: "array", items: ref("AuditLog") }) } });
  add("get", "/audit-logs", { summary: "Audit trail across all applications", description: "Includes events that belong to no application, such as an IP being locked out after repeated failed authentication.", tag: "Audit", auth: "admin", ok: { status: "200", response: json({ type: "array", items: ref("AuditLog") }) } });
  ...
  add("put", `${A}/risk-config`, {
    ...
    description: "Anything you leave out returns to its default. `sendCapPerHour` is a cost cap on OTP requests per hour that applies even when `mode` is `off`. `mode` is `off` (no evaluation), `log` (evaluate and record, never block; the default) or `enforce` (block for real). A score at or above `challengeScore` gives `challenge`; at or above `blockScore`, or any hard rule (blocked country, velocity limits), gives `block`.",
  ...
  return {
    ...
    tags: ["System", "Applications", "OTP", "Users", "API keys", "Webhooks", "Risk", "Audit", "Analytics", "Sessions", "Internal"].map((name) => ({ name })),
    paths,
    webhooks: {
    ...
    components: {
      ...
      schemas: {
        ...
        AuditLog: { type: "object", properties: { id: { type: "string" }, applicationId: { type: ["string", "null"] }, actorType: { enum: ["admin", "api_key", "system"] }, actorId: { type: ["string", "null"], description: "API key id when the actor is a key" }, action: { type: "string", examples: ["api_key.created", "webhook.secret_rotated", "risk_config.updated", "auth.lockout"] }, targetType: { type: ["string", "null"] }, targetId: { type: ["string", "null"] }, ipMasked: { type: ["string", "null"] }, metadata: {}, createdAt: { type: "string", format: "date-time" } } },
